...
const app=express();app.use(cors({origin:"*"}));app.use(express.json({limit:"32kb"}));
const server=http.createServer(app),io=new Server(server,{cors:{origin:"*",methods:["GET","POST"]}});
const pool=new Pool({connectionString:process.env.DATABASE_URL,ssl:process.env.DATABASE_URL?{rejectUnauthorized:false}:false});
const sessions=new Map(),rooms=new Map(),calls=new Map(),callReady=new Map();
...
function token(u){const t=crypto.randomBytes(32).toString("hex");sessions.set(t,u.email);return t}
...
async function auth(req,res){const t=String(req.headers.authorization||"").replace(/^Bearer /,"");const email=sessions.get(t);if(!email)return res.status(401).json({error:"Sessão inválida."});const u=await getUserByEmail(email);if(!u)return res.status(401).json({error:"Sessão inválida."});return u}
...
io.use(async(s,n)=>{try{const u=await getUserByEmail(sessions.get(s.handshake.auth?.token));if(!u)return n(new Error("Sessão inválida"));s.data.user=u;n()}catch(e){n(new Error("Falha na autenticação"))}});
io.on("connection",s=>{
...
initDb().then(()=>server.listen(process.env.PORT||3000,()=>console.log("Conversa Live v1.6 server ativo com PostgreSQL + 2FA por e-mail"))).catch(e=>{console.error("Falha ao iniciar banco:",e);process.exit(1)});
